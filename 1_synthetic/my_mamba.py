""" 
Изменённая реализация mamba из библиотеки mambapy
"""
...
# class MyMambaLogs:
#     """
#     класс для сохранения информации о внутренней работе mamba
#     """
#     def __init__(self):
#         self.deltas = None
...
class Mamba(nn.Module):
    ...
    # def get_deltas(self):  # мой код
    #     return torch.stack([res_block.mixer.logs['delta'] for res_block in self.layers], dim=2) # мой код
    
    def get_logs(self, name):  # мой код
        return torch.stack([res_block.mixer.logs[name] for res_block in self.layers], dim=2) # мой код
...
class MambaBlock(nn.Module):
    ...
    def ssm(self, x, z):
        ...
        self.logs['pred_delta'] = delta # мой код, (B, L, self.config.dt_rank)
        delta = self.dt_proj.weight @ delta.transpose(1, 2) # (ED, dt_rank) @ (B, L, dt_rank) -> (B, ED, L)
        # self.logs['delta'] = delta.transpose(1, 2) + self.dt_proj.bias # мой код, (B, L, ED)
        self.logs['delta'] = F.softplus(delta.transpose(1, 2) + self.dt_proj.bias) # мой код, (B, L, ED)
        self.logs['B'] = B
        self.logs['C'] = C
